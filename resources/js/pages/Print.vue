      ...
      <v-row class="my-5" v-if="have_access && kpiDataEncrypted">
        <div class="v-col-12"> 
          ...
          <div id="divToPrint" ref="pdfcont" width="100%" > 
            <div class="text-h6 mb-1 header-title text-center" style="text-align: center; margin-bottom: 10px;">GHASSAN ABOUD GROUP</div>
            <div class="sub-header py-1  mb-1 header-title text-center" style="text-align: center; margin-bottom: 10px;font-size:14px; !important">Performance Management System KPI's Form (Grade 6 - 10 Only)</div>
            <!-- <div class="mb-5 text-center" style="text-align: center; margin-bottom: 15px;font-size:12px;text-transform: uppercase;">
              ...
              <v-row class="download-print my-0">
                ...
                <v-col style="max-width:38%"  class="my-0 pb-0 pt-1">{{ kpiDataEncrypted.company ? kpiDataEncrypted.company.title : '' }}</v-col>  
            ...
            <table class="mt-1" v-if="kpiArray && kpiArray.length > 0" style="width:100%" border="1" cellpadding="0" cellspacing="0">
              <thead>
                <tr>
                  <td class="py-1 px-2" style="background:#e6e6e6;color:#000;" colspan="8">
                    KPI's count: 4 to 6 KPI's, 70% weightage to KPI's & 30% weightage to competencies development.  KPI's & Targets integrated with Annual Business Plan & Budget for FY {{year}}
                  </td>
                </tr>
                <tr>
              ...
              <tbody v-for="item,index in kpiArray" :key="item.id"> 
                  <tr> 
                    ...
                    <td class="px-2 text-center"> {{ item.achievement_midyear }}{{ item.achievement_midyear && item.measures == 'Percentage' ? '%' : '' }}</td>
                    <td class="px-2 text-center"> {{ item.achievement_yearend }}{{ item.achievement_yearend && item.measures == 'Percentage' ? '%' : '' }}</td>
          ...
          <v-row class="download-print my-0 mb-5"> 
          ...
          <div id="ecd-print" class="ecd-print">
            <div class="text-h6 mb-1 header-title text-center" style="text-align: center; margin-bottom: 10px;">GHASSAN ABOUD GROUP</div>
            <div class="sub-header py-1  mb-1 header-title text-center" style="text-align: center; margin-bottom: 10px;font-size:14px; !important">Performance Management System KPI's Form (Grade 6 - 10 Only)</div>
            
              <v-row class="download-print ma-0 mb-1"> 
                <v-col  md="12" sm="12" xs="12" class="px-4 py-2" style="border: 1px solid #000;">
                  Employee Capability Development - ECD (To be filled at the Time of KPI's Target Setting Phase Only, 30% weightage can be distributed between Technical & Behavioural areas)
                </v-col>
                <v-col md="6" sm="6" xs="6" class="px-4 py-2" style="border: 1px solid #000;">
                  <strong>Technical  / Functional / Job Related  Trainings:</strong> Identify 2 or 3  training programs along with weightage% which may help or increase  the employee job knowledge or competency level  to achieve the assigned target. (For Example: Business Analytics, Strategic Planning,  Essential Food Safety, Advance MS Office skills, Project Management, ECommerce, International Trade Terms, Supply chain Analytics , CRM, Business Intelligence, Advance Financial Management) 
                </v-col>
                <v-col md="6" sm="6" xs="6" class="px-4 py-2" style="border: 1px solid #000;">
                  <strong>Soft Skills or Behaviour Related Trainings:</strong> Identify 1 or 2 training programs weightage% which may enhance the  employee  behaviour or a skill  which enables to achieve the assigned target.  ( For example: Negotiation skills,  Coaching & Mentoring skills, Managerial skills development, Leadership Development, Planning & Organizing skills, Team Management & Supervisory development skills, Emotional Intelligence etc)
                </v-col>
                </v-row>
              ...
              <v-row class="download-print ma-0 mb-5"> 
                ...
                <v-col md="12" sm="12" xs="12" class="px-1 py-2" >
                  Final Rating (KPI's + Employee Capability Development): 5 - Top Performer,4- Strong Performer, 3- Competent Performer, 2- Developing Perfomer,1- Low Performer
                </v-col>
                <v-col md="6" sm="6" xs="6" class="px-1 py-4 d-flex" >Reporting   Manager's Signature & Date: <div style="border-bottom:1px solid #000;padding-right:170px;"></div></v-col>
                <v-col md="6" sm="6" xs="6" class="px-4 py-4 d-flex" >HOD or Reviewing  Manager's Signature & Date:<div style="border-bottom:1px solid #000;padding-right:165px;"></div></v-col>
                
                <v-col md="6" sm="6" xs="6" class="px-1 py-4 d-flex" >Employee  Signature & Date:<div style="border-bottom:1px solid #000;padding-right:220px;"></div></v-col>
                <v-col md="6" sm="6" xs="6" class="px-4 py-4 d-flex" >HRBP Signature & Date:<div style="border-bottom:1px solid #000;padding-right:280px;"></div></v-col>
             
              </v-row>
            </div>
          </div> 
          <v-btn size="small" color="primary" class="btn btn-primary noprint mr-1 mt-5" @click="() => printDocument(null,pdfcont,'portrait')">Download PDF - Portrait</v-btn>
          <v-btn size="small" color="primary" class="btn btn-primary noprint mr-1 mt-5" @click="() => printDocument(null,pdfcont,'landscape')">Download PDF - Landscape</v-btn>
          <v-btn size="small" color="primary" class="btn btn-primary noprint ml-1 mt-5" @click="() => printDocument(1)">PRINT</v-btn>
        </div>
      </v-row>  
...
<style>
.download-print{ font-size:10px;}
.sub-header{ background-color: #005795; color: #fff;}
  ...
  @media print{
    .v-container { background: #fff;}
    .ecd-print{ page-break-before:always}
    .noprint { 
      display:none !important;
      visibility:hidden !important
    }
    #divToPrint{ width:100% !important; max-width: 100%; }
    td,th { font-size: 10px !important; margin: 0 30px; }
    .secondary-title{ font-size: 10px !important; }
